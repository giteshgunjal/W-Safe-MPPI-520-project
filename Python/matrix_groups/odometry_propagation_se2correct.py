...
if __name__ == "__main__":
    ...
    print(np.sqrt(np.linalg.cholesky(np.eye(2))))
    dt = 0.04
    gt = {}
    ...
    robot['n'] = 500
    robot['x'] = []  # state mean
    robot['Cov_fo'] = np.diag([0.03 ** 2, 0.03 ** 2, 0.03 ** 2])  # first order covariance propagation around mean
    for i in range(robot['n']):
...
# def sigma_points(self):
        ...
        if pos_def_matrix(Sigma):
            ...
            if show_label:
                ...
                plt.plot(ELLIPSE_fo[:, 0], ELLIPSE_fo[:, 1], color=darkblue, alpha=0.7, linewidth=2)
                # plt.plot(ellipse[:, 0], ellipse[:, 1], color=DupontGray, alpha=0.7, linewidth=2)
        plt.plot(mu[0, 2], mu[1, 2], 'o', color=crimson, alpha=0.7, markersize=4)
        plt.legend()
        # plt.pause(0.05)
